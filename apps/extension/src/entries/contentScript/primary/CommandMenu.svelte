<script lang="ts">
    ...
    const commandMenuOpen = writable(false);
...
<Command.Dialog bind:open={$commandMenuOpen} {filter} {onKeydown} portal={null} preventScroll={false} closeOnOutsideClick={false} data-lenis-prevent>
    ...
    <Command.List>
		...
		{:else}
            {#each items?.saves as { title, description, url }}
